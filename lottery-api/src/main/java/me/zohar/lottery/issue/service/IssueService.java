...
public class IssueService {
	...
	public IssueVO mygetCurrentIssue(String gameCode) {
		Date now = new Date();
		Date bufferTime = DateUtil.offsetSecond(now, -5);

		Issue currentIssue = issueRepo.findTopByGameCodeAndStartTimeLessThanEqualAndEndTimeGreaterThanAndStateOrderByEndTimeAsc(
				gameCode, bufferTime, now, 1);
		if (currentIssue == null) {
			// 如果未找到当前期，可能在边界时间，需要手动获取最近的期
			currentIssue = issueRepo.findTopByGameCodeAndStartTimeLessThanEqualAndEndTimeGreaterThan(
					gameCode, now,now);
		}
		return IssueVO.convertFor(currentIssue);
	}
